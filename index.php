<?php 
$pageTitle="Become the healthier, fitter you with a MyTeam online fitness program.";
$pageDesc="12 week online programs help you lose weight, get stronger or get toned. Get coaching, simple weekly actions and communty support for £3.75 a week.";
include('inc/header.php'); ?>

  <div class="hero hero-home" id="top">
    <div class="w-container">
      <div class="w-row row_courseintro">
        <div class="w-col w-col-6 column_title">
          <h1 class="hero-title">Become the<br>healthier,<br>fitter you.</h1>
          <p class="paragraph-subtitle">12 week online coached
            <br>programs&nbsp;for £3.75 a week</p><a class="button button-hero" href="#current-course">Select a program</a>
        </div>
        <div class="w-col w-col-6">
          <div class="video"></div>
        </div>
      </div>
    </div>
  </div>
  <div class="section section-white" id="how-it-works">
    <div class="w-container">
      <h2>How it Works</h2>
      <div class="w-row row_benefits_1">
        <div class="w-col w-col-3 w-col-small-6">
          <img class="images_howitworks" src="images/MT_HowItWorks_1.png" width="154" alt="53204611c4db09226500097d_MT_HowItWorks_1.png">
          <h3 class="h3-feature">Choose your goal</h3>
          <p class="p-feature-description">Select a MyTeam online program based on your fitness goals.</p>
        </div>
        <div class="w-col w-col-3 w-col-small-6">
          <img class="images_howitworks" src="images/MT_HowItWorks_2.png" width="154" alt="53204621b6781caa760009f6_MT_HowItWorks_2.png">
          <h3 class="h3-feature">Join your team</h3>
          <p class="p-feature-description">Each program is run by a coach and has a supportive community of users.</p>
        </div>
        <div class="w-col w-col-3 w-col-small-6">
          <img class="images_howitworks" src="images/MT_HowItWorks_3.png" width="154" alt="5320462ab6781caa760009f8_MT_HowItWorks_3.png">
          <h3 class="h3-feature">Do your actions</h3>
          <p class="p-feature-description">Your coach will set you weekly tasks, making progress simple.</p>
        </div>
        <div class="w-col w-col-3 w-col-small-6">
          <img class="images_howitworks" src="images/MT_HowItWorks_4.png" width="154" alt="53204634475bc1a776000aac_MT_HowItWorks_4.png">
          <h3 class="h3-feature">Achieve your goal</h3>
          <p class="p-feature-description">With a coach and weekly actions, you’ll be successful in no time!</p>
        </div>
      </div>
    </div>
  </div>
  <div class="section section-gray" id="current-course">
    <div class="w-container">
      <h2>Current Programs</h2>
      <div class="w-row row-courses">
        <div class="w-col w-col-4">
          <div class="course-boundary">
            <div class="w-clearfix course-image">
              <h3 class="h3-course-name">Weight Loss Made Simple</h3>
              <div class="course-block-text descriptor">No exercise experience required</div>
            </div>
            <div class="w-clearfix trainer-profile">
              <img class="trainer-image-mini" src="images/Paul_Square.jpg" height="50" alt="5320a111c9fb84236500117e_Paul_Square.jpg">
              <div class="trainer-name-title"><strong>Paul Rayner<br></strong>Weight Loss Specialist</div>
            </div>
            <div class="course-summary">
              <ol class="w-list-unstyled">
                <li class="w-clearfix course-list">
                  <div class="course-block-text grey">For women serious about losing fat &amp; keeping it off.</div>
                </li>
                <li class="w-clearfix">
                  <div class="course-block-text grey below">No gym access required</div>
                </li>
              </ol><a class="button button-outline" href="weight-loss-made-simple">See the program</a>
            </div>
          </div>
        </div>
        <div class="w-col w-col-4">
          <div class="course-boundary">
            <div class="w-clearfix course-image strength">
              <h3 class="h3-course-name">Strength &amp;&nbsp;Size</h3>
              <div class="course-block-text descriptor">Basic lifting experience required</div>
            </div>
            <div class="w-clearfix trainer-profile">
              <img class="trainer-image-mini" src="images/Josh_Square.jpg" width="50" alt="5320a0f2475bc1a7760010fc_Josh_Square.jpg">
              <div class="trainer-name-title"><strong>Josh O’Leary<br></strong>Stength Coach</div>
            </div>
            <div class="course-summary">
              <ol class="w-list-unstyled">
                <li class="w-clearfix course-list">
                  <div class="course-block-text grey">A strength &amp; power course which builds size and shape.</div>
                </li>
                <li class="w-clearfix">
                  <div class="course-block-text grey below">Gym or weight set required</div>
                </li>
              </ol><a class="button button-outline" href="strength-size.html">See the program</a>
            </div>
          </div>
        </div>
        <div class="w-col w-col-4">
          <div class="course-boundary">
            <div class="w-clearfix course-image womens-fit">
              <h3 class="h3-course-name">Total<br>Body-Fit</h3>
              <div class="course-block-text descriptor">Intermediate fitness required</div>
            </div>
            <div class="trainer-profile">
              <div class="coming-soon-title">Coming Soon</div>
            </div>
            <div class="course-summary">
              <ol class="w-list-unstyled">
                <li class="course-list">
                  <div class="course-block-text grey">Women’s strength &amp; toning for the ultimate figure.</div>
                </li>
                <li class="w-clearfix">
                  <div class="course-block-text grey below">Basic kettlebell set required</div>
                </li>
              </ol><a class="button button-outline button-outline-grey" href="https://confirmsubscription.com/h/d/FD8602C3FEA606C1" onclick="ga(&apos;send&apos;, &apos;event&apos;, &apos;button&apos;, &apos;click&apos;, &apos;notify me&apos;);">Join waiting list</a>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
  <div class="call-out call-out-teal">
    <div class="w-container" id="trainerbio">
      <p class="p-call-to-action p-call-to-action-on-teal">Get notified about new programs coming up</p>
      <div class="support-copy">We are designing new programs for triathletes, runners, tough mudders and more.</div><a class="button button-forced-width" href="https://confirmsubscription.com/h/d/FD8602C3FEA606C1" onclick="ga(&apos;send&apos;, &apos;event&apos;, &apos;button&apos;, &apos;click&apos;, &apos;notify me&apos;);">Notify me!</a>
    </div>
  </div>


<?php include('inc/footer.php'); ?>	