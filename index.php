<?php 
$pageTitle="Affordable personal training";
$pageDesc="Reach your health and fitness goals through goal based communities powered by real coaches.";
include('inc/header.php'); ?>
		
<div class="scn_intro">
    <div class="w-container">
      <div class="w-container ctr_intro">
        <div class="w-row">
          <div class="w-col w-col-8 clm_intro">
            <h1 class="callout">Reach your 2014&nbsp;fitness goals with&nbsp;online trainers</h1>
          
            <div class="text-sub-home">12 week coached courses for £15 a month</div><a class="button home" href="#benefits">Learn more</a>
          </div>
          <div class="w-col w-col-4">
            <img class="w-hidden-small w-hidden-tiny iphone" src="images/MT_V5_iPhone.png" width="230" alt="52e0e733b37adb5912000866_MT_V5_iPhone.png">
          </div>
        </div>
      </div>
    </div>
    <div class="scn_popularcourses home_overlap">
      <div class="w-container ctr_popularcourses">
        <h1>Most popular courses</h1>
        <div class="w-row row_popularcourses">
          <div class="w-col w-col-4">
            <div class="blk_course">
              <img src="images/MT_V5_Img_WeightLoss.jpg" width="286" alt="52e1000675a811542d000b2f_MT_V5_Img_WeightLoss.jpg">
              <div class="blk_coursetitle">
                <h3 class="h3_coursetitle">Weight Loss<br>Made Simple</h3>
                <h4 class="h4_explevel">Beginner &nbsp;</h4>
                <div class="blk_trainer">
                  <div class="w-row">
                    <div class="w-col w-col-3 w-col-small-6 w-col-tiny-6 w-clearfix">
                      <img class="img_trainermini" src="images/MT_TrainerMini_Paul.png" width="50" height="50" alt="52e1015cece65a5a12000af7_MT_TrainerMini_Paul.png">
                    </div>
                    <div class="w-col w-col-9 w-col-small-6 w-col-tiny-6">
                      <div class="txt_trainerbio1">By Paul Rayner
                        <br>Weight Loss Specialist</div>
                    </div>
                  </div>
                </div>
                <div class="blk_coursedetail">
                  <div class="txt_coursesummary">For anyone wanting to lose weight and keep it off.</div><a class="button btn_grey btn_learnmore" href="weight-loss-made-simple.php" onclick="ga('send', 'event', 'button', 'click', 'wlms learn more home');">Learn more</a>
                </div>
              </div>
            </div>
          </div>
          <div class="w-col w-col-4">
            <div class="blk_course">
              <div class="image">
                <img src="images/MT_V5_Img_Strength.jpg" width="286" alt="52e1000e75a811542d000b30_MT_V5_Img_Strength.jpg">
              </div>
              <div class="blk_coursetitle">
                <h3 class="h3_coursetitle">Strength<br>and Size</h3>
                <h4 class="h4_explevel">Beginner - Intermediate</h4>
                <div class="blk_trainer">
                  <div class="w-row">
                    <div class="w-col w-col-3 w-col-small-6 w-col-tiny-6 w-clearfix">
                      <img class="img_trainermini" src="images/MT_TrainerMini_Josh.png" height="50" alt="52e10101ece65a5a12000aec_MT_TrainerMini_Josh.png">
                    </div>
                    <div class="w-col w-col-9 w-col-small-6 w-col-tiny-6">
                      <div class="txt_trainerbio1">By Joshua O'Leary
                        <br>Strength Coach</div>
                    </div>
                  </div>
                </div>
                <div class="blk_coursedetail">
                  <div class="txt_coursesummary">A strength and power course which builds size and shape.</div><a class="button btn_grey btn_learnmore" href="strength-size.php" onclick="ga('send', 'event', 'button', 'click', 'ss learn more home');">Learn more</a>
                </div>
              </div>
            </div>
          </div>
          <div class="w-col w-col-4 clm_popular3">
            <div class="blk_course">
              <img src="images/MT_V5_Img_Mum.jpg" width="286" alt="52e100191fd16d522d000ac0_MT_V5_Img_Mum.jpg">
              <div class="blk_coursetitle">
                <h3 class="h3_coursetitle">Yummy Mummy<br>Boot Camp</h3>
                <h4 class="h4_explevel">Beginner - Intermediate</h4>
                <div class="blk_trainer blk_trainerlight">
                  <h5>Coming in March</h5>
                </div>
                <div class="blk_coursedetail">
                  <div class="txt_coursesummary">For mums who want their body and energy back.</div><a class="button btn_grey btn_joinwaitlist" href="https://confirmsubscription.com/h/d/FD8602C3FEA606C1" onclick="ga('send', 'event', 'button', 'click', 'waiting list');">Join the wait list</a>
                </div>
              </div>
            </div>
          </div>
        </div><a class="button btn_teal" href="courses.php">See all courses</a>
      </div>
  
<?php
include('inc/content/benefits.php');
?>
      <div class="scn_getstarted">
        <div class="w-container ctr_getstarted">
          <h1 class="h1_white">Ready to get started?</h1><a class="button" href="courses.php">Choose your course</a>
        </div>
      </div>
    </div>


<?php include('inc/footer.php'); ?>


	